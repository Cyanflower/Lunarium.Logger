...
public class FilterBenchmarks
{
    ...
    private SinkOutputConfig _cfgNoRules = null!;
    private SinkOutputConfig _cfgWithIncludes = null!;
    private SinkOutputConfig _cfgWithExcludes = null!;
    ...
    public void Setup()
    {
        ...
        _cfgNoRules = new SinkOutputConfig();

        _cfgWithIncludes = new SinkOutputConfig
        {
            ContextFilterIncludes = ["Order", "Payment", "Auth", "User", "Http"]
        };

        _cfgWithExcludes = new SinkOutputConfig
        {
            ContextFilterExcludes = ["Debug.Internal", "System", "Microsoft"]
        };

        _filterNoRules = new LoggerFilter();
        _filterWithIncludes = new LoggerFilter();
        _filterWithExcludes = new LoggerFilter();
        ...
        // 预热缓存，确保命中测试时直接走缓存路径
        _filterNoRules.ShouldEmit(_entryMatchingContext, _cfgNoRules);
        _filterWithIncludes.ShouldEmit(_entryMatchingContext, _cfgWithIncludes);
        _filterWithIncludes.ShouldEmit(_entryNonMatchingContext, _cfgWithIncludes);
        _filterWithExcludes.ShouldEmit(_entryMatchingContext, _cfgWithExcludes);
        _filterWithExcludes.ShouldEmit(_entryNonMatchingContext, _cfgWithExcludes);
    }
    ...
    public bool NoRules_CacheMiss_Approx()
    {
        ...
        _missIndex++;
        return _filterNoRules.ShouldEmit(entry, _cfgNoRules);
    }
}

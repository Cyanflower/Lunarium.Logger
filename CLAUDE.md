...
├── SinkOutputConfig.cs      # 单个 Sink 配置（级别过滤、上下文过滤、ToJson?、IsColor?、TextOutputIncludeConfig?）
...
│
├── Target/
│   ├── ILogTarget.cs        # ILogTarget（Emit+IDisposable）、IJsonTextTarget、ITextTarget（含 IsColor + TextOutputIncludeConfig）、ILogEntryTarget
...
`ToJson: bool?`、`IsColor: bool?` 和 `TextOutputIncludeConfig: TextOutputIncludeConfig?` 均为可空类型，`null` 表示"不覆盖目标的默认值"：
...
> ⚠️ `LogWriter.Render(entry, config)` 重载目前已实现但各 Target 的 `Emit()` 方法尚未使用（仍调用 `Render(entry)`）。`TextOutputIncludeConfig` 属性已挂载到 Target，待后续 `Emit()` 改造后生效。
...
- **行覆盖率**: 91.5%（2287/2499 行）
- **分支覆盖率**: 88.2%（703/797 分支）
- **方法覆盖率**: 98.8%（355/359 个方法）
...
- [x] `UseLunariumLog()` 扩展方法（`ILoggingBuilder`，自动构建 Logger + 注册 Provider + 单例生命周期管理；适用于 `builder.Logging.UseLunariumLog(...)` 和 `hostBuilder.ConfigureLogging(l => l.UseLunariumLog(...))`）
...
- [x] `IColorTextTarget` 重命名为 `ITextTarget`，新增 `TextOutputIncludeConfig` 属性（控制 Timestamp/Level/LoggerName/Context 各字段可见性）
- [x] `LogWriter.Render(entry, TextOutputIncludeConfig)` 重载（按配置选择性渲染字段；各 Target 的 Emit() 暂未使用此重载）
- [x] `ConsoleTarget`/`FileTarget`/`StringChannelTarget`/`ByteChannelTarget` 均实现 `ITextTarget`
...
- `Sink` 构建时自动探测目标是否实现接口，若 `SinkOutputConfig.ToJson?/IsColor?/TextOutputIncludeConfig?` 非 null 则强制注入
...
| `SinkOutputConfig.ToJson?/IsColor?/TextOutputIncludeConfig?` 可空 | `null`=不覆盖目标默认；非 null=强制覆盖 |

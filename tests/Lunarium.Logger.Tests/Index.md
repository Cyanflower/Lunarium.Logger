...
- **主要测试场景**:
  - **默认值验证（第1节，1个测试）**：`LogMinLevel` 为 `Info`、`LogMaxLevel` 为 `Critical`、`ToJson` 为 `null`、`ContextFilterIncludes/Excludes` 为 `null`、`IgnoreFilterCase` 为 `false`、`TextOutputIncludeConfig` 为 `null`。
...
- **主要测试场景**:
  ...
  - **Sink.SetJsonSinkProperty — IsColor/TextOutputIncludeConfig 路径（第5节，6个测试）**：`IsColor=false` 设置 `ITextTarget`；`IsColor=true` 保持 `true`；`IsColor=null` 不修改；Target 不实现 `ITextTarget` 时不抛异常（使用 `Substitute.For<ILogTarget>()`）；`TextOutputIncludeConfig` 非 null 时设置到 target；`TextOutputIncludeConfig=null` 时不修改。
...
- **主要测试场景**:
  ...
  - `StringChannelTarget_TextOutputIncludeConfig_CanBeSet`：`TextOutputIncludeConfig` 属性可设置并读取正确值。
...
- **主要测试场景**:
  ...
  - **AddFileSink（第3节，4个测试）**：无 config 重载返回同一 Builder（含 DisposeAsync）；带 `SinkOutputConfig` 返回同一 Builder；Build 后写一条日志验证文件存在且内容正确；`sinkOutputConfig: null` 不抛异常。
  - **AddConsoleSink（第3b节，2个测试）**：无参和带 `SinkOutputConfig` 重载均返回同一 Builder 实例。

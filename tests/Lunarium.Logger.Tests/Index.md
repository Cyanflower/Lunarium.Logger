...
- **主要测试场景**:
  - **默认值验证（第1节，1个测试）**：`LogMinLevel` 为 `Info`、`LogMaxLevel` 为 `Critical`、`ToJson` 为 `null`、`ContextFilterIncludes/Excludes` 为 `null`、`IgnoreFilterCase` 为 `false`。
  ...
  - **ToJson 标志（第4节，1个测试）**：设为 `true` 后读取正确。
...
- **目标类**: `InternalLogger`（位于 `Lunarium.Logger.InternalLoggerUtils` 命名空间）
- **主要测试场景**:
...
- **主要测试场景**:
  ...
  - **Logger.GetContext（第9节，3个测试）**：`GetContext()` 返回空字符串；`GetContextSpan()` 解码后等于构造时的 `loggerName`；空名称时 `GetContextSpan()` 为空。
...
- **目标类**: `BufferWriter`（位于 `Lunarium.Logger.Internal` 命名空间）
- **主要测试场景**:
...
- **目标类**: `DestructureHelper`（位于 `Lunarium.Logger` 命名空间）
...
- **主要测试场景**:
  - **Sink 构造与解构（第1节，4个测试）**：带/不带 `SinkOutputConfig` 的构造；2-out 和 3-out `Deconstruct` 重载均正确拆解 `Target`、`Configuration`、`LoggerFilter`。
  - **ConsoleSinkConfig（第2节，3个测试）**：默认 `SinkOutputConfig` 为 null；设置后正确；实现 `ISinkConfig` 接口。
  - **FileSinkConfig（第3节，3个测试）**：验证 `MaxFileSizeMB`、`RotateOnNewDay`、`MaxFile`、`SinkOutputConfig` 的默认值及完整赋值；实现 `ISinkConfig` 接口。
  - **Sink.SetJsonSinkProperty — ToJson 路径（第4节，3个测试）**：`ToJson=true` 将 `IJsonTextTarget` 的属性设为 `true`；`ToJson=false` 覆写默认值；`ToJson=null` 不修改 Target 原有值。
  - **Sink.SetJsonSinkProperty — IsColor 路径（第5节，4个测试）**：`IsColor=false` 设置 `IColorTextTarget`；`IsColor=true` 保持 `true`；`IsColor=null` 不修改；Target 不实现 `IColorTextTarget` 时不抛异常。
...
- **主要测试场景**:
  ...
  - `StringChannelTarget_ToJson_CanBeSet`：`ToJson` 属性可以被设置并读取正确值。
...
- **主要测试场景**:
  ...
  - **上下文（第4节，3个测试）**：有上下文时可见且使用 Cyan(`\x1b[96m`)；空上下文时不出现 Cyan 码。
...
- **说明**: 使用内部 `CapturingLogger` 具体实现（非 NSubstitute mock）捕获 `Log()` 调用，确保 DIM 能通过 `ILogger` 接口类型引用正确触发。
- **主要测试场景（17个测试）**:
...
- **目标类**: `LoggerExtensions`（`ForContext`/`ForContext<T>`）、`LoggerBuilderExtensions`（`AddConsoleSink`、`AddStringChannelSink`、`AddLogEntryChannelSink`、`AddChannelSink<T>`、`AddSink(ISinkConfig)`、`AddTimedRotatingFileSink`、`AddSizedRotatingFileSink`、`AddRotatingFileSink`）
- **主要测试场景**:
  ...
  - **AddConsoleSink（第3节，2个测试）**：无参和带 `SinkOutputConfig` 重载均返回同一 Builder 实例。
  - **AddStringChannelSink（第4节，4个测试）**：基本重载返回 Builder 和非 null Reader；带 capacity 和 isColor 参数的重载；`toJson=true` 分支；带 `SinkOutputConfig` 分支。
  - **AddSink(ISinkConfig)（第5节，4个测试）**：`ConsoleSinkConfig` 分发；`FileSinkConfig` 分发（临时路径，Build 后 DisposeAsync）；自定义 `ISinkConfig` 通过 `CreateTarget()` 注册（NSubstitute 验证调用次数）。
  - **AddTimedRotatingFileSink / AddSizedRotatingFileSink（第6节，2个测试）**：各返回同一 Builder，Build 后可正常 DisposeAsync。
  - **AddLogEntryChannelSink（第7节，3个测试）**：基本/带 capacity/带 SinkOutputConfig 重载各返回 Builder 和非 null Reader。

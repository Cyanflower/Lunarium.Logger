...
namespace Lunarium.Logger.Tests.Filter;
...
public class LoggerFilterTests
{
    ...
    private static bool ShouldEmit(LogEntry entry, SinkOutputConfig cfg)
    {
        var filter = new LoggerFilter();
        return filter.ShouldEmit(entry, cfg);
    }
    ...
    public void ShouldEmit_SameContext_CalledTwice_ConsistentResult()
    {
        ...
        var filter = new LoggerFilter();
        var first = filter.ShouldEmit(entry, cfg);
        var second = filter.ShouldEmit(entry, cfg);

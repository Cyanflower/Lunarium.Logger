...
namespace Lunarium.Logger.Tests.Integration;
...
public class LoggerFixture : IAsyncDisposable
{
    ...
    public LoggerFixture()
    {
        ConcreteLogger = new LoggerBuilder()
            ...
            .AddSink(
                new StringChannelTarget(ErrorOnlyChannel.Writer, isColor: false),
                new SinkOutputConfig { LogMinLevel = LogLevel.Error, LogMaxLevel = LogLevel.Critical })
            .AddSink(
                new StringChannelTarget(ServiceOnlyChannel.Writer, isColor: false),
...
public class ChannelTargetDirectTests
{
    ...
    {
        var ch = Channel.CreateUnbounded<string>();
        var target = new StringChannelTarget(ch.Writer, isColor: true);
